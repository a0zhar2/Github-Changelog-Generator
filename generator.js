...
document.getElementById("userinfo").addEventListener("click", function () {
  ...
  if (username == "" || repository == "") {
    ...
    main_container.innerHTML = '<div class="alert alert-warning">One of the Input boxes are empty</div><br>'+bak;
    return;
  }
  ...
  async function displayCommits() {
    ...
    try {
      ...
      commits.forEach((commit) => {
        ...
        changelog += generate_markdown_changelog(commit.commit.message.split("\n"));
      });

      // Add the completed changelog to the commitsList element
      commitsList.innerHTML += changelog;
    } catch (error) {
      commitsList.innerHTML = '<div class="alert alert-danger">Error fetching commits. Please try again later.</div><br>'+bak;
    }
  }
